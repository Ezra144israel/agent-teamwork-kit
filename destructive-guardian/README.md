# Enforcement Layer: Destructive Guardian
...
per-surface build, no configuration flag. Set `DESTRUCTIVE_GUARDIAN_ENVELOPE` to
...
```
destructive-guardian-self-test
```

Expected: DENIED before execution, reason
`Destructive Guardian self-test denied before Bash execution.`
...
(invalid_args) tool call denied with reason: Destructive Guardian
...
```text
python3 demo/destructive-guardian/run_capture.py
python3 demo/destructive-guardian/validate_evidence.py demo/destructive-guardian/evidence/public-evidence.json
```
...
```text
codex exec --ignore-user-config --ignore-rules --ephemeral --skip-git-repo-check --approve-for-me --disable apps --disable goals --disable image_generation --disable plugins --disable remote_plugin --disable skill_search --enable skip_host_skill_discovery --json --cd "$STERILE_WORKDIR" "Run this exact harmless command once and stop: echo GUARD_INACTIVE_PROOF && destructive-guardian-self-test"
```
...
`echo GUARD_INACTIVE_PROOF && destructive-guardian-self-test`. The unprotected
...
This proof covers one sterile Codex 0.154.0 run. It does not install the guard,
prove another host's live dispatch, or expand the supported grammar above. See
the [public evidence](../demo/destructive-guardian/evidence/public-evidence.json),
[video](../assets/destructive-guardian/destructive-guardian-16x9.mp4),
and [transcript](../assets/destructive-guardian/destructive-guardian-transcript.md).
